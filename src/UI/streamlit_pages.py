        ...
        if game_choice:
            ...
            if game_code:
                ...
                if game_metadata:
                    ...
                    # Execute the Python code to render the game
                    st.subheader(f"Play {game_choice}:")
                    ...
                    # Play the game
                    try:
                        exec(game_code)  # Executing the Python code generated by LLM
                    except Exception as e:
                        st.error(f"An error occurred while executing the game: {str(e)}")
                        st.text(game_code)  # Display the code for debugging
                else:
                    st.error("The generated code did not pass validation. Please check the game generation.")
            else:
                st.error(f"Failed to generate the game for '{game_choice}'. Please try again.")
        else:
            st.error("Please enter a game name.")